...
public class DAOPedido extends Conexion implements CRUD<Pedido>{
    ...
    public void registrar(Pedido t) throws Exception {
        try{
            this.conectar();
            PreparedStatement st =  this.conexion.prepareStatement("INSERT INTO `pedido` (`cod_pedido`, `fecha_hora`, `estado`, `cod_empleado`,`cod_mesa`, `direccion`,`fecha_hora_c`) VALUES (?, NOW(), ?, ?, ?, '',NOW());");
            st.setString(1, t.getCodigoP());
            st.setString(2, t.getEstadoPed().name());
            st.setString(3, t.getDempleado().getCodigoE());
    ...
    public List<Pedido> listar() throws Exception {
        ...
        try{
            this.conectar();
            PreparedStatement st =  this.conexion.prepareStatement("SELECT `pedido`.`cod_pedido`, `mesa`.`descripcion`,`pedido`.`direccion`, `factura`.`subtotal`, `pedido`.`estado`, `pedido`.`fecha_hora`, `pedido`.`fecha_hora_c` \n" +
"FROM `pedido` INNER JOIN `factura` ON `pedido`.`cod_pedido` = `factura`.`cod_pedido`\n" +
"JOIN `mesa` ON `pedido`.`cod_mesa`=`mesa`.`cod_mesa` ORDER by `pedido`.`fecha_hora`");
            lista = new ArrayList<>();
            ...
            while(rs.next()){
                ...
                pedido.setFechaHoraC(rs.getDate("fecha_hora_c").toLocalDate());
                lista.add(pedido);
            }
            rs.close();
            st.close();
